...
\citep{hommel-arslan-2025, pellert-etal-2026}: \valRangeCosMin--\valRangeCosMax{} over all
\valNPairs{} pairs, even opposite-keyed ones (Figure~\ref{fig:corplot},
left). Negatives must be manufactured (sign-flipping, $\valRangeArMin$,
and \texttt{squid}, $\valRangeSqMin$), whereas \texttt{mean\_centered\_pearson}
...
(range = \valRangeCosMin--\valRangeCosMax), with darker within-domain blocks (most visibly the
...
NLI scores E1 and its reverse-keyed sibling E2 at $\valNliOpp$,
near-perfect contradiction, versus $\valArOpp$ under sign-flipping, which
...
sfa_embed: 50 x 1024 in 114.3 s
...
cosine $\geq \valLiveCosMin$, reproducing the archived output on-device.
